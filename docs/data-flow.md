...
- **What gets pulled** — every `.md`, `.markdown`, `.rst` file at any depth in
  ...
  filtered out before any fetch. There is no per-project *content* cap, but
  the GitLab tree walk caps **discovery** at 2000 entries per project as a
  safety guard against pathological repos (huge mono-repos with autogenerated
  trees). Anything beyond that is silently skipped — declare a more specific
  source if a project legitimately exceeds it.
...
```mermaid
flowchart TD
    START[Connector yields DocumentRef] --> LIST[List all upstream docs]
    LIST --> TOMB[Tombstone removed:<br/>delete OS chunks +<br/>registry rows for paths<br/>no longer in upstream]
    TOMB --> FETCH[Fetch each document]
...
Edges come in two flavours stored in the same `kg_dependencies` table:

- **Catalog (`kind = "service"`)** — `to_service_id` references another row
  in `services`. Selected from a team-grouped, searchable Combobox.
- **External (`kind = "external"`)** — `to_external_name` + optional
  `to_external_description` capture a free-text target outside the catalog
  (Stripe, Auth0, an upstream team's API not yet declared, etc.). Stored
  in the same row schema with `to_service_id IS NULL`. The CHECK
  constraint enforces XOR; case-insensitive uniqueness is enforced by a
...
flowchart LR
    ...
    API -->|service_repo.add_dependency<br/>or add_external_dependency| DB[(kg_dependencies)]
    API -. catalog edge invalidates .-> GRAPH[/api/organization/graph]
    UI -->|DELETE /services/:id/dependencies/:to| API
    UI -->|DELETE /services/:id/dependencies/external?name=...| API
    API -->|remove_dependency<br/>or remove_external_dependency| DB
```
...
- The org graph (`OrganizationGraph.tsx`) reads `dependencies` from
  `/api/organization/graph` and renders dashed `depends on` edges. External
  rows are filtered out of `list_all_dependencies` because the graph only
  renders nodes for declared catalog entities — externals are visible only
  on the originating service's detail page.
- The blast-radius graph on analyses uses the same dataset to walk
  upstream/downstream from the assigned team's services. The recursive
  CTE skips external rows naturally because the JOIN to `services` on
  `to_service_id` drops them.
