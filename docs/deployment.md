...
| `PRISM_LOCAL_SOURCE_ROOT` | `./data` (relative to backend cwd; `run.sh` exports the repo-root absolute path) | filesystem jail for path-based connectors (sharepoint / excel / onenote). Every `config.path` must resolve inside this subtree -- `..` traversal and symlink-escape are rejected. Default-on as of round 11. `run.sh` exports `<repo>/data` so the local script and the documented top-level `data/` agree; compose sets `/data`. |
| `PRISM_CORS_ORIGINS` | `http://localhost:5173,http://127.0.0.1:5173` | comma-separated list of allowed browser origins for the API. The previous wildcard + credentials default was unsafe even pre-auth because it would silently inherit into any future cookie/token browser flow. Production deployments override with their actual UI origin(s). Set to `*` to bypass the boundary entirely (credentials are then disabled per the CORS spec). |
