...
CREATE TABLE IF NOT EXISTS analyses (
    ...
    -- 4-8 word headline summarizing the requirement. Generated by the
    -- planner on first touch so long paragraph-length requirements don't
    -- wrap awkwardly in turn card headers. Nullable: empty/null falls back
    -- to the raw requirement in the UI.
    title TEXT
);
...
class AnalysisRepository:
    ...
    async def update_title(self, analysis_id: str, title: str) -> None:
        # Title is write-once on first successful generation; no-op if empty.
        if not title:
            return
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE analyses SET title = $2 WHERE analysis_id = $1",
                analysis_id,
                title,
            )
    ...
    async def list_thread(self, thread_id: str) -> list[dict]:
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT analysis_id, requirement, title, status, created_at, duration_seconds,
